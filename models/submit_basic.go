...
type SubmitBasic struct {
	...
	ProblemIdentity string        `gorm:"column:problem_identity;type:varchar(36);" json:"problem_identity"` // 问题表唯一标识
	ProblemBasic    *ProblemBasic `gorm:"foreignKey:identity;references:problem_identity"`                   // 关联问题基础表
	UserIdentity    string        `gorm:"column:user_identity;type:varchar(36);" json:"user_identity"`       // 用户表唯一标识
	UserBasic       *UserBasic    `gorm:"foreignKey:identity;references:user_identity"`                      // 关联问题基础表
	Path            string        `gorm:"column:path;type:varchar(255)" json:"path"`                         // 代码存放路径
	Status          int           `gorm:"column:status;type:tinyint(1)" json:"status"`                       // 【-1	-待判断，1-答案正确，2-答案错误，3-运行超市，4-运行超内存】
}
