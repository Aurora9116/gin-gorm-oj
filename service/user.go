...
func Login(c *gin.Context) {
	...
	token, err := helper.GenerateToken(data.Name, data.Identity, data.IsAdmin)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
			"msg":  err.Error(),
...
func SendCode(c *gin.Context) {
	...
	err := helper.SendEmail([]string{email}, code)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
...
func Register(c *gin.Context) {
	...
	// 生成token
	token, err := helper.GenerateToken(name, userIdentity, data.IsAdmin)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
